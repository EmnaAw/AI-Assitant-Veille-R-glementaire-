...
QUESTIONS = [
    "A partir de combien de salariés l'entreprise doit désigner un responsable sécurité ?",
    "Quel le rôle d'un responsable sécurité au travail ?",
    "Quel est le rôle du commission Consultative d'entreprise CCE ?",
    "Quelle est le fréquence des réunions du CCE ?",
    "Qui sont les membres du comité santé et sécurité au travail ?",
    "Qui remplise la fiche d'entreprise ?",
    "Quelles sont les fonctions de médecin du travail dans l'entreprise ?",
    "Quelles sont les obligations de l'entreprise pour la gestion des déchets dangereux ?",
    "Par quelle instructions les collaborateurs des déchets dangereux doivent êtres agréés ?",
    "Quelles sont les obligations d'une entreprise pour la mesure de la qualité de l'air ?",
    "Quelles sont les entreprises qui doivent réaliser une étude de dangers ?",
    "Le dossier de classement d'un établissement dangereux doit être déposé à quelle institution ?",
    "Qui valide l'étude de dangers ?",
    "Qui valide l'étude d'impact sur l'environnement ?",
    "Quelle réglementation concerne attestation de prévention des risques incendie, explosion et panique ?",
    "Quelle réglementation concerne un registre de sécurité ?",
    "Quelle réglementation concerne l'autorisation d'exploitation d'un établissement classé ?",
    "Quelle réglementation concerne la déclaration de procédés du travail provoquant des maladies professionnelles ?",
    "Quelle réglementation est liée à la Gestion de quelques produits chimiques dangereux non conforme ?",
    "Quelle réglementation concerne l'autorisation d'ouverture d'un établissement classé de la 3ème catégorie ?",
    "Quelle réglementation concerne attestations d'approvisionnement des produits chimiques ?",
    "Quelle veille réglementaire est liée à : Manque de port des EPI pour les travailleurs ?",
    "Quelle réglementation concerne l'accord pour l'exploitation des données Biométriques et des caméras de surveillance ?",
    "Quelle veille réglementaire est liée à l'analyse de la qualité d'eau rejeté ?",
    "Quelle réglementation concerne attestation de conformité des installations électriques ?",
    "Quelle réglementation concerne étalonnage des équipements de mesure : Télémètre, appareil de mesure d'isolement ?",
    "Quelle veille réglementaire est liée à : Régime horaire non approuvé par l'inspection de travail ?",
    "Quelle veille réglementaire est liée au nombre insuffisant d'installations sanitaires ?",
    "Quelle réglementation concerne un espace Fumeur et manque d'affichage ?",
    "Quelle veille réglementaire est liée à : Responsable SST non encore nommé ?",
    "Quelle réglementation concerne la liste nominative ?",
    "Quelle veille réglementaire est liée à : La société n'a pas encore son attestation de prévention ?",
    "Quelle veille réglementaire est liée à : Fiche alimentaire barquette ?",
    "Quelle réglementation concerne une équipe de première intervention ?",
    "Quelle réglementation concerne les fiches de surveillance médicales spéciales ?",
    "Quelle réglementation concerne conteneur spécifique pour la collecte des déchets de soins ?",
    "Quelle réglementation concerne contrat avec les transporteurs des matières dangereuses ?",
    "Quelle réglementation est liée au Régime horaire de travail ?",
    "Quelle réglementation concerne les personnes handicapées ?",
    "Quelle réglementation concerne la fiche de nomination du responsable sécurité ?",
    "Quelle réglementation concerne une équipe de seconde intervention ?",
    "Quelle réglementation concerne les accords pour l'exploitation des données à caractère personnel ?",
    "Quelle réglementation concerne un plan intérieur d'intervention ?",
    "Quelle réglementation concerne les analyses des rejets hydriques ?",
    "Quelle réglementation concerne la formation en geste et posture pour la manutention manuelle des charges ?",
    "Quelle réglementation concerne la déclaration de procédés du travail provoquant MP ?",
    "Quelle réglementation concerne les RIA (Robinets d'Incendie Armés) au sein des sites de l'entreprise ?",
    "Quelle veille réglementaire est liée à la Gestion des pneus usagées ?",
    "Quelle réglementation concerne le rapport de l'activité annuelle du médecin de travail ?",
    "Quelle réglementation concerne formation au profit des agents de gardiennage ?",
]

_app_label_embeddings = None
...
def _get_app_embeddings(emb_model) -> dict[str, np.ndarray]:
...
def classify_query(query: str, emb_model):
    ...
    if best_score >= CLASSIFY_THRESHOLD and best_label:
        ...
        return {"app_id": app_id, "label": best_label, "score": best_score}
...
def build_source_header(meta, index):
...
class LocalRAG:
    ...
    def retrieve_docs(self, user_query: str):
        ...
        domain_filter = classify_query(user_query, self.emb)
        if domain_filter:
            clauses.append({"app_id": domain_filter["app_id"]})
            debug["domain_label"] = domain_filter["label"]
            debug["domain_score"] = round(domain_filter["score"], 2)
            debug["app_id"] = domain_filter["app_id"]
        else:
            debug["domain_label"] = None
            debug["domain_score"] = None
            debug["app_id"] = None
        ...
        debug["exact_object_keys"] = exact_keys
        ...
        debug["doc_count"] = len(docs)

        return docs, debug

    def answer(self, user_query: str):
        docs, debug = self.retrieve_docs(user_query)

        if not docs:
            return "Je n'ai pas l'information dans le contexte fourni.", [], debug
...
### Reponse:"""
        ...
        try:
            ...
            answer_text = f"ERREUR_LLM: {e}"

        return answer_text, list(sources_seen.keys()), debug
...
def run_batch():
    ...
    for i, q in enumerate(QUESTIONS, 1):
        print(f"[{i:02d}/{len(QUESTIONS)}] {q[:90]}...")
        try:
            answer, sources, debug = rag.answer(q)
            print("   ✓ OK")
        except Exception as e:
            answer = f"ERREUR: {e}"
            sources = []
        ...
        output_lines.append(f"R{i}. {answer}")

        if sources:
            output_lines.append(f"S{i}. {' | '.join(sources)}")

        if debug:
            output_lines.append(
                f"D{i}. domaine={debug.get('domain_label')} "
                f"score={debug.get('domain_score')} "
                f"app_id={debug.get('app_id')} "
                f"fallback={debug.get('fallback_used')} "
                f"docs={debug.get('doc_count')} "
                f"exact={debug.get('exact_object_keys')}"
            )
    ...
    with open("answers_copy_paste.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(output_lines))

    print("\n✓ Fichier généré: answers_copy_paste.txt")
